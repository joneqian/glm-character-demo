...
st.set_page_config(page_title="CharacterGLM API Demo",
...
# 设置API KEY
api_key = st.sidebar.text_input("API_KEY", value=os.getenv(
    "API_KEY", ""), key="API_KEY", type="password", on_change=update_api_key)
...
with st.container():
    ...
    with col2:
        st.text_input(label="用户名", value="用户", key="user_name", on_change=lambda: st.session_state["meta"].update(
            user_name=st.session_state["user_name"]), help="用户的名字，默认为用户")
        st.text_area(label="用户人设", value="", key="user_info", on_change=lambda: st.session_state["meta"].update(
            user_info=st.session_state["user_info"]), help="用户的详细人设信息，可以为空")

    with col3:
        st.selectbox(label="图片风格", options=["二次元风格", "写实风格", "赛博朋克风格", "国画风格"], key="image_style", on_change=lambda: st.session_state["meta"].update(
            image_style=st.session_state["image_style"]), help="生成图片的风格")
...
def draw_new_image():
    ...
    if text_messages:
        ...
        # 若没有对话历史，则根据角色人设生成图片
        image_prompt = "".join(generate_role_appearance(
    ...
    # TODO: 加上风格选项
    image_prompt = st.session_state["meta"]["image_style"] + \
        '。' + image_prompt.strip()
    ...
    for i in range(n_retry):
        try:
            ...
            break
    img_msg = ImageMsg(
        {"role": "image", "image": img_url, "caption": image_prompt})
...
with st.container():
    ...
    with button_key_to_col["clear_history"]:
        clear_history = st.button(
            button_labels["clear_history"], key="clear_history")
        if clear_history:
            init_session()
            st.rerun()

    with button_key_to_col["gen_picture"]:
        gen_picture = st.button(
            button_labels["gen_picture"], key="gen_picture")

    if debug:
        with button_key_to_col["show_api_key"]:
            show_api_key = st.button(
                button_labels["show_api_key"], key="show_api_key")
            if show_api_key:
        ...
        with button_key_to_col["show_history"]:
            show_history = st.button(
                button_labels["show_history"], key="show_history")
            if show_history:
...
def start_chat():
    ...
    if not query:
        ...
        response_stream = get_characterglm_response(filter_text_msg(
            st.session_state["history"]), meta=st.session_state["meta"])
        bot_response = output_stream_response(
            response_stream, message_placeholder)
        if not bot_response:
